...
def ver() -> str:
    ...
    if (not mydb.ask.exist(school, email)) or (
        token != mydb.ask.get(school, email).split(";")[1]
    ):
...
def supporter() -> str | flask.Response:
    ...
    return flask.render_template(
        "supporter.html", pop_title="Successed", pop_msg="Successed!", pop_type="ok"
    )
